...
enum SignupStatus { none, loading, done }

class SignupState extends Equatable {
  SignupStatus status;
  bool error;

  SignupState([this.status = SignupStatus.none, this.error = false]);
...
class SignupBloc extends Bloc<SignupEvent, SignupState> {
  ...
  void _submitForm(SubmitSignupForm event, Emitter<SignupState> emitter) async {
    emitter.call(SignupState(SignupStatus.loading));

    try {
      Client client = await authRepository.signup(Client(
          -1,
          User(-1, event.email, event.email, false, password: event.password),
          event.firstName,
          event.middleName,
          event.lastName,
          event.sex,
          ...
          event.country,
          event.phoneNumber));
      emitter.call(SignupState(SignupStatus.done));
    } catch (e) {
      log("Error: ${(e as ApiException).response?.body}");

      emitter.call(SignupState(SignupStatus.none, true));
    }
  }
}
